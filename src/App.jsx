...
import { CopyPlus, Search, ChevronDown, ImageUp, Loader2, LogOut } from 'lucide-react';
...
function applyDatabaseChange(rows, payload) {
  ...
  if (!id) return rows;
  if (payload.eventType === 'DELETE') return rows.filter(row => row.id !== id);
  const next = payload.new;
  const existingIndex = rows.findIndex(row => row.id === id);
  if (existingIndex === -1) return [next, ...rows];
...
function AddSongModal({ open, onClose, onSubmit, initial }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit({
        ...(isEdit ? { id: initial.id, expected_updated_at: initial.updated_at } : {}),
        title: title.trim(),
        artist: artist.trim(),
        status,
        tabs_link: tabsLink.trim(),
      ...
      setSaveError(error?.message || 'Could not save this song');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label={isEdit ? 'Edit song' : 'Add song'} className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4">
      {/* Backdrop with blur */}
      <div
      ...
      {/* Modal Card */}
      <div
        className="relative z-10 flex max-h-[100dvh] w-full max-w-[720px] flex-col overflow-hidden rounded-t-[24px] sm:max-h-[calc(100dvh-2rem)] sm:rounded-[24px]"
        style={{
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-[36px] flex flex-col">
                ...
                <button
                  ...
                  onClick={async () => {
                    try {
                      ...
                      else setSaveError("No cover found");
                    } catch (error) {
                      setSaveError(error?.message || "Could not fetch artwork");
                    } finally {
          ...
          <div className="flex items-center justify-end gap-4 mt-6">
            <Button variant="glass" disabled={saving} onClick={() => { reset(); onClose(); }}>
              Cancel
            </Button>
...
function SongsTab({ items, loading, onEdit }) {
  ...
  const aggregated = useMemo(() => {
    ...
    for (const s of items) {
      ...
      if (!map.has(key)) map.set(key, { _key: key, id: s.id || key, title: s.title || "", artist: s.artist || "", status: s.status || SONG_STATUSES[0], statuses: new Set(), artwork_url: s.artwork_url || "", tabs_link: s.tabs_link || "", video_link: s.video_link || "", recording_link: s.recording_link || "", notes: s.notes || "", updated_at: s.updated_at || "" });
      const g = map.get(key);
      if (s.status) {
...
export default function App() {
  ...
  useEffect(() => {
    if (!loadingSongs && !loadingLessons) {
      writeLibraryCache(LESPAL_WORKSPACE_ID, songs, lessons);
    }
  ...
  useEffect(() => {
    ...
    window.addEventListener('pageshow', refreshLibrary);
    return () => {
      document.removeEventListener('visibilitychange', refreshLibrary);
      window.removeEventListener('pageshow', refreshLibrary);
    };
  }, [loadSongs, loadLessons]);

  useEffect(() => {
    if (realtimeStatus === 'SUBSCRIBED') return undefined;
    const intervalId = window.setInterval(() => {
      loadSongs({ silent: true });
      loadLessons({ silent: true });
    }, 30000);
    return () => window.clearInterval(intervalId);
  }, [realtimeStatus, loadSongs, loadLessons]);
  ...
  const handleUpsertSong = async (payload) => {
    ...
    setSongs(rows => applyDatabaseChange(rows, { eventType: 'UPDATE', new: saved, old: {} })
      .sort((a, b) => (a.title || '').localeCompare(b.title || '')));
    return saved;
  };
  ...
  const handleUpsertLesson = async (payload) => {
    ...
    setLessons(rows => applyDatabaseChange(rows, { eventType: 'UPDATE', new: saved, old: {} })
      .sort((a, b) => String(b.date || '').localeCompare(String(a.date || ''))));
    return saved;
  };
  ...
  const handleDeleteLesson = async (id) => {
    ...
    setLessons(rows => rows.filter(lesson => lesson.id !== id));
  };
        ...
        <div className={`mx-auto max-w-[1440px] px-[36px] py-[24px] flex items-center justify-between h-[92px] transition-all duration-300 ${isMobile ? 'px-[20px] h-auto flex-wrap gap-4' : ''}`}>
          ...
          <div className="flex items-center gap-[12px]">
            <Button
              ...
              size="icon"
              aria-label={tab === 'lessons' ? 'Add lesson' : 'Add song'}
              onClick={() => {
          ...
          {isMobile && (
            <div className="w-full relative rounded-full backdrop-blur-md">
              ...
              />
              <div className="flex items-center justify-center gap-12 px-[32px] py-[14px] rounded-full bg-transparent relative z-10">
